...
var names=countries.map(n=>n.name)
...
function countrydetail(){
  cname=id_select.value
  cdata=countries.find(c=>c.name==cname)
countrydetailcard(cdata)
}

function countrydetailcard(cdata){
  let htmldata=`<div class="card mt-3" style="width: 100%;">
  <img src="${cdata.flag}" class="card-img-top" alt="...">
  <div class="card-body">
    <h5 class="card-title">${cdata.name}</h5>
  </div>
  <ul class="list-group list-group-flush">
    <li class="list-group-item">${cdata.capital}</li>
    <li class="list-group-item">${cdata.population}</li>
    <li class="list-group-item">${cdata.currencies[0].name}</li>
  </ul>
</div>`
